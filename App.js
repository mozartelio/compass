...
export default function App() {
  ...
  const [magnom_state, setMagnom_state] = useState('0');
  ...
  const subscribe = async () => {
    ...
    setSubscription(
      magnetometer.subscribe((sensorData, error) => {
        setMagnom_state(angler(sensorData));
        console.log(
      ...
      <View style={styles.container}>
        <Image
          style={{
            width: 340,
            height: 340,
            transform: [{rotate: 360 - magnom_state + 'deg'}],
          }}
          // source={require("./assets/compass.png")}
          source={require('./assets/compass_turned.png')}
        />

        <Text style={styles.degreeText}>{degree(magnom_state) + '°'}</Text>
      </View>
    </View>
  );
}
